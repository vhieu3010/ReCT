...
class Eagle_Loss(nn.Module):
    ...
    def __init__(self, patch_size, cpu=False, cutoff=0.5):
        ...
        self.kernel_x = nn.Parameter(
            torch.tensor(kernel_values, dtype=torch.float32).unsqueeze(
                0).unsqueeze(0).to(self.device),
            requires_grad=False)
        self.kernel_y = nn.Parameter(
            torch.tensor(kernel_values, dtype=torch.float32).t(
            ).unsqueeze(0).unsqueeze(0).to(self.device),
            requires_grad=False)

        # Operation for unfolding image into non-overlapping patches
        self.unfold = nn.Unfold(
            kernel_size=self.patch_size, stride=self.patch_size).to(self.device)
    ...
    def calculate_patch_loss(self, output_gradient, target_gradient):
        output_gradient, target_gradient = output_gradient.to(
            self.device), target_gradient.to(self.device)
    ...
    def gaussian_highpass_weights2d(self, size):
        freq_x = torch.fft.fftfreq(
            size[0]).reshape(-1, 1).repeat(1, size[1]).to(self.device)
        freq_y = torch.fft.fftfreq(size[1]).reshape(
            1, -1).repeat(size[0], 1).to(self.device)
...
class Eagle_Loss_3D(nn.Module):
    ...
    def gaussian_highpass_weights_3d(self, size):
        freq_x = torch.fft.fftfreq(
            size[0]).reshape(-1, 1, 1).repeat(1, size[1], size[2]).to(self.device)
        freq_y = torch.fft.fftfreq(size[1]).reshape(
            1, -1, 1).repeat(size[0], 1, size[2]).to(self.device)
        freq_z = torch.fft.fftfreq(size[2]).reshape(
            1, 1, -1).repeat(size[0], size[1], 1).to(self.device)
