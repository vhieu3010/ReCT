...
def main():
    ...
    train_dataset = FOVDataset(
        './fov_extension_data/train', transform=transform)
    val_dataset = FOVDataset('./fov_extension_data/val', transform=transform)
    test_dataset = FOVDataset('./fov_extension_data/test', transform=transform)
    ...
    for criterion in loss_list:
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (data, groundtruth) in enumerate(train_loader):
                data, groundtruth = data.to(device), groundtruth.to(device)
                optimizer.zero_grad()
                outputs = model(data)
            ...
            if val_loader:
                ...
                with torch.no_grad():
                    for data, groundtruth in val_loader:
                        data, groundtruth = data.to(
                            device), groundtruth.to(device)
                        outputs = model(data)
                        loss = criterion(outputs, groundtruth)
        ...
        with torch.no_grad():
            for i, (inputs, labels) in enumerate(test_loader):
                if i == 41:  # choose a specific image
                    ...
                    # first output image
                    output_img = outputs[0].cpu().squeeze()

                    image_names = ['Ground_Truth',
                                   f'reconstruction_{loss_name}']
                    ...
                    for i, img in enumerate(
                        ...
                        main_image_path = os.path.join(
                            results_dir, f"{image_names[i]}_main.png")
                        ...
                        plt.tight_layout()
                        main_image_path = os.path.join(
                            results_dir, f"{image_names[i]}_ori.png")
                        plt.savefig(main_image_path)
                        plt.close(fig)
                        ...
                        ax.axis('off')
                        ax.set_xlim(
                            zoom_coords[0], zoom_coords[0] + zoom_coords[2])
                        ax.set_ylim(zoom_coords[1] +
                                    zoom_coords[3], zoom_coords[1])
                        plt.tight_layout()
                        zoomed_image_path = os.path.join(
                            results_dir, f"{image_names[i]}_zoomed.png")
        ...
        # Calculate metrics
        (ssim_avg, ssim_std), (psnr_avg, psnr_std), (mse_avg,
                                                     mse_std) = metrics_with_std(model, test_loader, device)
        results_str = f"""
